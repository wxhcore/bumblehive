...
_EXEC_PARAMETERS: dict[str, Any] = {
    ...
    "properties": {
        ...
        "working_dir": {
            "type": "string",
            "description": (
                "Optional working directory. Relative paths resolve from the workspace; "
...
class ExecRunner:
    ...
    def _prepare_command(
        ...
        if isinstance(cwd, str):
            return {"error": cwd}
        guard_error = self._guard_deny_patterns(command)
        if guard_error:
    ...
    def _resolve_working_dir(
        ...
        if working_dir:
            resolved = access.resolve_path(working_dir)
        else:
